...
# modified from bookops-worldcat to adhere to Python 2
...
class WorldcatAccessToken:
    ...
    def _get_token_url(self):
        return "{}/token".format(self.oauth_server)
    ...
    def _parse_server_response(self, response):
        ...
        if response.status_code == requests.codes.ok:
            ...
            jres = response.json()
            raise APITokenError(
                "Authorization server error code: {}, "
                "error message: {}".format(jres["code"], jres["message"])
            )
