...
class AccessToken(object):
    ...
    validGrantTypes = ["authorization_code", "refresh_token", "client_credentials"]
    ...
    def __init__(self, authorization_server, grant_type=None, options=None):
        ...
        if grant_type is None or grant_type not in AccessToken.validGrantTypes:
            raise InvalidGrantType(
                "You must pass a valid grant type to construct an Access Token."
            )
        self.grant_type = grant_type

        if not options:
            raise NoOptionsPassed(
                "You must pass at least one option to construct an Access Token. Valid options "
        ...
        if self.grant_type == "authorization_code" and (
            ...
            or not "authenticating_institution_id" in options
            or not "context_institution_id" in options
            or not "scope" in options
        ):
            raise RequiredOptionsMissing(
        ...
        if "scope" in options and not isinstance(options["scope"], list):
            raise RequiredOptionsMissing(
                "scope must be a list of one or more scopes, i.e. ['WMS_NCIP' {, ...}]"
            )
    ...
    def is_expired(self):
        ...
        status = False
        if (
            time.mktime(time.strptime(self.expires_at, "%Y-%m-%d %H:%M:%SZ"))
            < time.time()
        ):
            status = True
        return status
    ...
    def create(self, wskey, user=None):
        ...
        authorization = self.wskey.get_hmac_signature(
            method="POST", request_url=self.access_token_url, options=self.options
        )
        print(authorization)
    ...
    def refresh(self):
        ...
        self.access_token_url = self.get_access_token_url()
        authorization = self.wskey.get_hmac_signature(
            method="POST", request_url=self.access_token_url
        )
        self.request_access_token(authorization, self.access_token_url)
    ...
    def get_access_token_url(self):
        ...
        access_token_url = (
            self.authorization_server + "/accessToken?grant_type=" + self.grant_type
        )
        if self.grant_type == "refresh_token":
            ...
            access_token_url += (
                ...
                + self.authenticating_institution_id
                + "&"
    ...
    def parse_token_response(self, response_string):
        ...
        try:
            ...
            print(
                "ValueError: Unable to decode this Access Token response string to JSON:"
            )
        ...
        if (
            ...
            and principal_idns is not None
            and not principal_id == ""
            and not principal_idns == ""
        ):
            self.user = User(
...
$wskey"""
            ...
            {
                "access_token_url": self.access_token_url.replace(
                    "?", "?\n" + " " * 18
                ).replace("&", "\n" + " " * 18 + "&"),
